...
public class ChatBotServiceImpl implements ChatBotService {
    ...
    public FunctionResponse getFunctionResponse(String prompt) {
        ...
        System.out.println("Response Body from GEMINI API: " + responseBody);

        FunctionResponse res = new FunctionResponse();

        try {
            // Check if "functionCall" exists in the response
            if (ctx.read("$.candidates[0].content.parts[0].functionCall", Object.class) != null) {
                ...
                // Set values in FunctionResponse
                res.setCurrencyName(currencyName);
                res.setCurrencyData(currencyData);
                res.setFunctionName(functionName);
            } else {
                // Fallback: Extract plain text if "functionCall" is not present
                System.out.println("FunctionCall not present. Defaulting to plain text response.");
                String textResponse = ctx.read("$.candidates[0].content.parts[0].text");
                res.setFunctionName("NA"); // No function was called
                res.setCurrencyName("NA");
                res.setCurrencyData(textResponse); // Use plain text as fallback
            }
        } catch (Exception e) {
